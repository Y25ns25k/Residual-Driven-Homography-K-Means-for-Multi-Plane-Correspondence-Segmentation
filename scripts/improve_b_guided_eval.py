"""Improvement B evaluation: guided full-set discovery (+ ICM combo) on AdelaideRMF-H.

Variants:
  residual_hkm_no_merge_guided          local-sampling full-set proposals,
                                        conservative gate (margin 0.1), retry x3
  residual_hkm_no_merge_guided_icm      + ICM Potts smoothing (lambda_s=0.5)
Usage: python improve_b_guided_eval.py [seed ...]
"""
...
out_dir = ROOT / "outputs" / "improve_b"
...
for seed in SEEDS:
    ...
    for scene in scenes:
        ...
        km = ResidualHomographyKMeans(config, random_state=hkm_seed, use_guided_discovery=True, **NO_MERGE)
        fit = km.fit(x1, x2, image_shape=image_shape)

        variants = {"residual_hkm_no_merge_guided": (fit.labels, fit.runtime)}
        ...
        if fit.homographies:
            ...
            smoothed = fit.labels
        variants["residual_hkm_no_merge_guided_icm"] = (smoothed, fit.runtime + time.perf_counter() - t0)
                ...
                rows.append(
                    {
                        ...
                        "discovery_accepted": fit.diagnostics.get("discovery_accepted", 0),
                        **metrics,
                    }
                )
...
pd.DataFrame(rows).to_csv(out_dir / f"metrics_guided_seeds_{tag}.csv", index=False)
