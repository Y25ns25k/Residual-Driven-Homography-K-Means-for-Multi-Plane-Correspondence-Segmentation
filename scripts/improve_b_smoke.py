"""Improvement B smoke test: split on barrsmith/elderhallb (undersegmented scenes)."""
...
for scene_id in ("barrsmith", "elderhallb", "nese", "bonython"):
    scene = by_id[scene_id]
    seed = _adelaide_method_seed(123, scene.scene_id, "residual_hkm_no_merge")
    for split in (False, True):
        km = ResidualHomographyKMeans(config, random_state=seed, use_residual_split=split, **NO_MERGE)
        ...
        d = fit.diagnostics
        print(f"{scene_id:>12} split={split}: K_gt={int(m['K_gt'])} K_est={int(m['K_est'])} "
              f"ME={100*m['ME']:.2f}% runtime={fit.runtime:.1f}s "
              f"split_attempts={d.get('split_attempts')} accepted={d.get('split_accepted')}")
